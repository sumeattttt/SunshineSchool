...
const CARDS = [
  {
    ...
    description:
      "Sensory space, learning space and life skill training.",
    href: "/facilities",
    icon: Building2,
...
export function QuickInfo() {
  return (
    <section className="container-x py-20 md:py-28">
      ...
      <StaggerContainer staggerDelay={0.1} className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {CARDS.map(({ eyebrow, title, description, href, icon: Icon, accent }) => (
          <StaggerItem key={title}>
            <Link
              href={href}
              className="group relative flex h-full flex-col justify-between overflow-hidden rounded-3xl border border-border bg-card p-6 transition-all hover:-translate-y-1.5 hover:border-brand-ink/20 hover:shadow-xl hover:shadow-black/5"
            >
              <div
                aria-hidden
